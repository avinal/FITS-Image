    ...
    else
    {
        ...
        while (cptr)
        {
            if (strlen(cptr) > maxlen)
                maxlen = strlen(cptr); /* find longest token */

            /* name contains special characters? */
            tstatus = -1; /* suppress any error message */
            if (fftkey(cptr, &tstatus) > 0)
                specialchar = 1;
    ...
    *hdtype = 0;
    ...
    else /* no negative sign at beginning of template */
    {
      ...
      */
        ...
        if (!FSTRCMP(keyname, "COMMENT") || !FSTRCMP(keyname, "HISTORY") || !FSTRCMP(keyname, "HIERARCH"))
        {
...
*/
    ...
    for (pat = 0; pat < npat; pat++)
    {
        ...
        m1 = -1;
        n1 = -1;
        a = ' '; /* Initialize the place-holders */
        pass = 0;
...
*/
{
    ...
    for (jj = 1; jj <= (fptr->Fptr)->tfield && *status <= 0; jj++)
    {
        ...
        for (ii = 1; ii <= (fptr->Fptr)->numrows; ii++)
        {
            ...
            {
                if (valid)
                    *valid = FALSE; /* address out of bounds */
                snprintf(message, FLEN_ERRMSG,
...
*/
{
    ...
    else if ((fptr->Fptr)->datastart == DATA_UNDEFINED)
    {
        ...
        delta = (((fptr->Fptr)->headend + (morekeys * 80)) / 2880 + 1) * 2880 - (fptr->Fptr)->datastart;
...
int ffcdfl(fitsfile *fptr, int *status)
{
    ...
    filpos = (fptr->Fptr)->datastart + (fptr->Fptr)->heapstart + (fptr->Fptr)->heapsize;
...
*/
{
...

    {
        ...
        nshift = (long)(((fptr->Fptr)->headstart[(fptr->Fptr)->maxhdu + 1] - insertpt) / 2880);
...
*/
{
    ...
    if (!decimalpt)
    {                       /* only do this once for efficiency */
